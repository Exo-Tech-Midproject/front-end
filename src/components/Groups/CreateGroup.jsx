...
//   //       let allGroups = await axios.get(`${DBRUL}/physician/${payload.username}/groups`,
//   //           {
...
//   //       let token = cookie.load('auth')
//   //       const payload = await jwtDecode(token)
//   //       let allGroups = await axios.post(`${DBRUL}/physician/${payload.username}/groups`,groupData,
//   //           {
//   //               headers: { Authorization: `Bearer ${token}` }
//   //           })
//   //       console.log(allGroups.data)
...
export default function CreateGroup() {
  ...
  const handleDescriptionChange = (e) => {
    ...
    if (inputValue.length < 5 || inputValue.length > 50) {
      setDescriptionError('Description must be between 5 and 50 characters.');
    } else {
      setDescriptionError(null);
    }
  };
